...
namespace RFQ.UI.Infrastructure.Provider
{
    public class BookingOrTripAdaptor : IBookingOrTripAdaptor
    {
        ...
        public BookingOrTripAdaptor(HttpClient httpClient, GlobalClass globalClass, IConfiguration configuration)
        {
        ...
        public async Task<PageList<BookingOrTripResponseDto>> GetAllBookingOrTrip(PagingParam pagingParam)
        {
            try
            {
                using (var httpClient = new HttpClient())
                {
                    ...
                    var baseUrl = _fleetLynkApiUrl + _config["BookingOrTrip:GetAllBookingOrTrip"];
                    var response = await httpClient.PostAsync(baseUrl, requestContent);
                    var responseData = await response.Content.ReadAsStringAsync();
                    var responseModel = JsonConvert.DeserializeObject<CommanResponseDto>(responseData);
                    if (responseModel?.Data?.result != null)
                    {
                        var bookingOrTripList = JsonConvert.DeserializeObject<List<BookingOrTripResponseDto>>(
                        ...
                        return new PageList<BookingOrTripResponseDto>(bookingOrTripList, totalRecordCount, pageNumber, pageSize);
                    }
                    return null;
                }
            }
